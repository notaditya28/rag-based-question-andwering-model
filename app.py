...
DOCUMENTS = [
    ...
    },
    {
        ...
        "text": (
            "CUDA is a parallel computing platform developed by NVIDIA for GPU programming. "
            "It allows developers to use the GPU for general purpose computing tasks. "
            "CUDA programs are written in C/C++ with special extensions. "
            "Threads are organized into blocks and grids for parallel execution. "
            "GPU computing is much faster than CPU for tasks like matrix multiplication."
        ),
    },
    {
        "id": "doc7",
        ...
        "text": (
            ...
            "Each device trains a local model and only shares model updates. "
            "NVIDIA FLARE is a popular framework for federated learning. "
            "It preserves data privacy while still enabling collaborative model training."
        ),
    },
]
...
# ── UI ───────────────────────────────────────────────────
st.title("🤖 RAG Agent — Building RAG Agents with LLMs")
st.caption("Ask anything about AI/ML topics from the knowledge base")
    ...
    with st.chat_message(message["role"]):
        ...
        if "chunks" in message:
            with st.expander("📄 Retrieved Sources"):
                for rank, (score, chunk) in enumerate(message["chunks"], 1):
                    color = "🟢" if score > 0.3 else "🟡" if score > 0.15 else "🔴"
                    st.markdown(f"**{color} [{rank}] {chunk['doc_title']}** — score: `{score:.3f}`")
                    st.caption(chunk["text"])

# Chat input
if prompt := st.chat_input("Ask something about AI/ML..."):
    if not api_key:
    ...
    # RAG pipeline
    kb, idf = get_knowledge_base()
    retrieved = retrieve(prompt, kb, idf, top_k=3)

    context = "\n\n".join([
        f"[{i+1}] (Source: {c['doc_title']}, score={s:.3f})\n{c['text']}"
